...
export class MetricsCollector {
     ...
     */
    recordQuery(query: string, results: any[], duration: number): void {
     ...
     */
    recordEmbedding(docCount: number, dimension: number, duration: number): void {
     ...
     */
    recordRetrieval(query: string, k: number, resultsCount: number, duration: number): void {
     ...
     */
    recordGeneration(model: string, promptLength: number, responseLength: number, duration: number): void {
     ...
     * Record error
     */
    recordError(operation: string, error: Error | any, context: object = {}): void {
    ...
    exportPrometheus(): string {
        const summary: any = this.getSummary();
        const lines: string[] = [];
    ...
    // Helper methods
    private _avg(arr: any[], key: string): string {
        if (arr.length === 0) return '0';
        return (this._sum(arr, key) / arr.length).toFixed(2);
    }

    private _sum(arr: any[], key: string): number {
 ...
 */
export function measured(type: 'query' | 'embedding' | 'retrieval') {
